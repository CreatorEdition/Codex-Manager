...
- 2026-06-19 只读诊断确认更新检测默认地址仍是 `qxcnm/Codex-Manager`：`apps/src-tauri/src/commands/updater/runtime.rs` 的 `DEFAULT_UPDATE_REPO`、设置页 fallback release URL、环境变量目录默认值仍指向上游；需统一改为 CreatorEdition 发布源，并保留 `CODEXMANAGER_UPDATE_REPO` 覆盖能力。
- 2026-06-19 只读诊断确认当前分支存在大量前端 conflict marker，涉及 `apps/package.json`、`apps/tests/transport-web-commands.test.mjs`、`apps/src/hooks/useAccounts.ts`、`apps/src/app/apikeys/page.tsx`、`apps/src/app/logs/page.tsx`、`apps/src/app/settings/page.tsx`、i18n 消息文件等；这是构建与运行时验证的前置阻塞，必须先按来源提交逐文件收敛，不能继续在冲突态上判断前端性能效果。
...
- 2026-06-19 只读诊断发现 `task.md` 记录的 `account/usage/aggregate` SQL 下推在当前 `hardening/main` 未实际生效：`crates/core/src/storage/usage.rs` 已有 `usage_aggregate_summary()`，但 `crates/service/src/usage/usage_aggregate.rs` 仍 `list_accounts()` + `latest_usage_snapshots_by_account()` 后在 Rust 聚合；需接回 storage 下推函数并补 regression test 防止分支回退。
...
- 2026-06-19 只读诊断确认 token refresh 已有 6 小时失败退避和 `refresh_token_invalid:*` 永久过滤，但 `refresh_token_unknown_401` 也会被标记为永久无效；这类可能是服务端抖动或身份服务临时异常，后续应把 reused / invalidated / expired / invalid_grant / app_session_terminated 作为永久类，unknown_401、网络错误、5xx 走指数退避与有限探测，不直接永久停刷。
