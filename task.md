...
- 上游差异已重新拉取复核：`upstream/main` 最新为 `6ac01a2a`；已按语义移植 `2c912580`、`84ac0560`、`ce5d3f38`、`83ca26f7`/`359580a7`、`4ba9a139`、`99bb0c2d`、`0508c19a`、`6ac01a2a`、`03ca2052` 等功能项；`09223f6f`、`f3efb3a2` 仍需按 CE 约束逐项评估，禁止整包合并上游推广/作者内容。
...
### ✅ 已完成：P2 平台密钥今日用量统计语义移植

- 目标：语义移植上游 `4ba9a139` 的平台密钥当日用量展示，但不采用上游全量 Key+模型扫描路径。
- 行为：`apikey/usageStats` 返回 `todayTokens` 与 `todayEstimatedCostUsd`；管理员/成员仍沿用 CE 当前 `keyIds` 限载和权限过滤，当前页只统计当前页 Key，空 keyIds 继续返回空数组。
- 前端：平台密钥表格的“Token / 金额”列拆成“今日”和“累计”两段；累计额度上限、剩余额度和费用口径保持不变。
- 验证：`cargo test -p codexmanager-core --lib api_key_usage_stat_summary_serialization_uses_camel_case -- --nocapture` 通过；`cargo test -p codexmanager-service --test rpc rpc_apikey_usage_stats_filters_requested_key_ids -- --exact --nocapture` 通过；`apps` 下 `tsc --noEmit` 通过；`cargo fmt --all --check` 通过；`git diff --check` 通过。

### ✅ 已完成：P2 托盘恢复导航语义移植

- 目标：语义移植上游 `99bb0c2d` 的桌面端托盘恢复导航修复，但不合并上游额外窗口状态机改造。
- 行为：主窗口轻量关闭后再次从托盘、单实例唤起或系统命令恢复时，会标记这是“已有主窗口后的重建”；debug 模式下重建窗口会显式导航到桌面开发服务根页，避免停留在启动/占位页面。生产构建保持 no-op，不改变已打包页面加载路径。
- 验证：`cargo test --manifest-path apps/src-tauri/Cargo.toml recreated_main_window_navigation_only_runs_for_debug_recreate -- --nocapture` 通过；`cargo check --manifest-path apps/src-tauri/Cargo.toml` 通过。

### ✅ 已完成：P2 Web 启动设置读取优化

- 目标：语义移植上游 `0508c19a`，但按 CE 当前 Web gateway、startup snapshot 与 RPC 超时矩阵实现，不直接复制上游代码。
- 行为：Web gateway 模式读取应用设置的冷启动等待窗口从 15 秒提高到 60 秒；桌面和其他运行模式仍保持 15 秒，避免普通启动异常被过长掩盖。
...
- 目标：语义移植上游 `03ca2052` 中适合 CE 当前架构的低风险性能项，不整包套用上游 before-build 代理重构和 UI 样式细节。
- 行为：Next dev 模式忽略 `node_modules`、`src-tauri/target` 和 `.pnpm-store` 监听并降低轮询压力；`.pnpm-store/` 已加入 `.gitignore`。
- 行为：AppBootstrap、Header、Sidebar、日志页改为按字段订阅 Zustand store；store setter 对无变化 patch 返回原状态，减少启动和运行中的无效重渲染。
- 行为：托盘预览启动快照禁用 API 模型预取，复用 CE 现有 `includeApiModels` 轻量参数；CE 现有 `accountLimit`、`apiKeyLimit`、`includeUsageAggregate`、`includeTodaySummary` 已覆盖上游部分启动快照轻量化目的。
- 验证：`node --test tests\dashboard-direct-mode.test.mjs` 通过；`apps` 下 `tsc --noEmit` 通过；`cargo fmt --all --check` 通过；`git diff --check` 通过；冲突标记扫描无命中。

### 📌 后续待完成任务

1. P2：上游差异语义评估与 PR/分支治理；当前 fork 与 upstream 分叉较大，对外 PR 应从干净分支 cherry-pick 关键提交，不建议整包提交。
2. P2：评估是否移植上游 `09223f6f`/`f3efb3a2` 产品设计重构和 UI 密度优化；每项必须先剥离上游作者/推广内容并按 CE 当前性能改造复核。
