...
3. P2 低优先级性能观察
   ...
   - 请求体 JSON parse 深水区继续观察：本地校验、多候选 `prompt_cache_key` 提取、compact transport、非原生 Responses 默认 `stream=true` 后文本长度校验复用已收敛；request rewrite、local count tokens、WebSocket 包装等路径仍需按风险继续拆小项评估。
