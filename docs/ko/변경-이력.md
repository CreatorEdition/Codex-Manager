...
### Fixed
- `v0.3.10` 릴리스 산출물이 여전히 `0.3.8` 파일명으로 생성되던 문제를 수정하고 workspace, frontend package, Tauri desktop metadata, lock file 버전을 동기화했습니다.
- 기존 tag를 갱신할 때 release action이 이전 CodexManager 산출물을 먼저 삭제하여 같은 GitHub Release 안에 오래된 버전 파일이 남지 않게 했습니다.
