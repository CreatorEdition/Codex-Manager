...
**证据**：
1. 同一账号，CLI 直连正常，经过代理就降级
2. `reasoning_tokens=516` 不是随机值，像是一个预设的"降级阈值"
3. 用户提到"IP 信誉"和"账号风控"相关

**推测机制**：
```
OpenAI 服务端检测到：
- 请求来自非官方客户端（User-Agent / Originator 不匹配）
- 或请求经过代理（IP 信誉、请求模式异常）
→ 触发 "Reasoning Budget Limit"
→ 强制截断 reasoning 过程到 516 tokens
→ 导致推理不完整，返回错误答案
```

### 推测 2: CE 侧 SSE 解析时序问题
