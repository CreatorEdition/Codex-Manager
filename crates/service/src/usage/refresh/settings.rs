...
use super::{
    ...
    DEFAULT_USAGE_REFRESH_WORKERS, DEFAULT_WARMUP_WORKERS, ENV_DISABLE_POLLING,
    ...
    USAGE_REFRESH_WORKERS, USAGE_REFRESH_WORKERS_ENV, WARMUP_CRON_ENABLED,
    WARMUP_CRON_EXPRESSION, WARMUP_CRON_SIGNAL, WARMUP_WORKERS, WARMUP_WORKERS_ENV,
};
...
fn reload_background_tasks_from_env() {
    ...
    WARMUP_WORKERS.store(
        env_usize_or(WARMUP_WORKERS_ENV, DEFAULT_WARMUP_WORKERS).max(1),
        Ordering::Relaxed,
    );
