use crate::app_settings::{
    list_app_settings_map, listener_bind_addr_for_mode, normalize_optional_text,
    APP_SETTING_ENV_OVERRIDES_KEY,
};
...
fn save_bool_setting_if_changed(settings: &HashMap<String, String>, key: &str, value: bool) {
    save_app_setting_if_changed(settings, key, Some(if value { "1" } else { "0" }));
}
...
fn persist_get_snapshot_if_changed(
    ...
    save_app_setting_if_changed(settings, APP_SETTING_UI_THEME_KEY, Some(theme));
    save_app_setting_if_changed(
        settings,
        APP_SETTING_UI_APPEARANCE_PRESET_KEY,
        Some(appearance_preset),
    );
    save_app_setting_if_changed(settings, APP_SETTING_UI_LOCALE_KEY, Some(locale));
